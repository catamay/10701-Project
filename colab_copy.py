...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            # Directly return the continuous action predicted by the model
            return model(state).cpu().squeeze(0)
    else:
        # Sample a random action from the action space (continuous)
...
def optimize_model():
    ...
    # Compute the MSE loss between the predicted actions and the actual actions taken
...
env = RecordVideo(env, video_folder="Half-Cheetah", name_prefix="eval", episode_trigger=lambda x: True)
state, info = env.reset()
state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
with torch.no_grad():
    for t in count():
